 ...
 * called before {@code release()}, UsbCameraHolder will try to hold the {@code
...
public class UsbCameraHolder {
            ...
            switch(msg.what) {
                case RELEASE_CAMERA:
                    synchronized (UsbCameraHolder.this) {
                        // In 'UsbCameraHolder.open', the 'RELEASE_CAMERA' message
                        ...
                        // here:
                        if (UsbCameraHolder.this.mUsers == 0) releaseCamera();
                    }
                    break;
            }
        }
    ...
    private UsbCameraHolder() {
        HandlerThread ht = new HandlerThread("UsbCameraHolder");
        ht.start();
        mHandler = new MyHandler(ht.getLooper());
        
        // use standard android camera api instead
        //mNumberOfCameras = android.hardware.Camera.getNumberOfExternalCameras();
        ...
        for (int i = 0; i < mNumberOfCameras; i++) {
            ...
            // use standard android camera api instead
            //android.hardware.Camera.getExternalCameraInfo(i, mInfo[i]);
        ...
        if (mCameraDevice == null) {
            try {
                if(mCameraDevice == null){
                    ...
                    // use standard android camera api instead
                    //mCameraDevice = android.hardware.Camera.openExternal(cameraId);
